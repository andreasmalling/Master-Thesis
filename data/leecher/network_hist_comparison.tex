...
%        x tick label style={rotate=60, anchor=east},
    ...
    \end{tikzpicture}
    \caption[Mean bandwidth per client from Leecher exps.]{Mean of accumulated bandwidth for each type of client for; \newline
    (1) a network with 5 Binge (\expid{B5}), \newline
    (2) a network with 5 Leecher and 5 Binge (\expid{L5B5}). \newline 
    Leechers are more efficient in the amount of data they download while Binges downloads duplicate data, Binges had very similar amount of received data regardless if there are Leechers present or not.}
