\begin{figure}[ht]
    \myfloatalign
    \begin{tikzpicture}
        \begin{axis}[
            ybar,
            ymin=0,
        %        bar width = 4pt,
            xtick=data,
            xticklabels from table={./data/global/stall_hist.csv}{users},
            x tick label style={rotate=60, anchor=east},
            ylabel={\# Stalls},
            legend style={at={(1.05,1)}, anchor=north west,legend columns=1},]
            \addplot table [x expr=\coordindex, y={stalls}]{./data/global/stall_hist.csv};
        \end{axis}
    \end{tikzpicture}
    \caption[Total number stalls from \expid{B10-g}]{Total number of stall in the \expid{B10-g}, half of the users experienced stalls other than the initial stall.}
    \label{bar:global_stall_hist}
\end{figure}