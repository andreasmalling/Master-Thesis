...
Since the hash addresses always will be the same for the content, it is possible to pre-generate and add the addresses to the \acs{MPD} before any files are shared in \acs{IPFS}, no matter which client latter will add them (See section \ref{sec:ipfs-objects} for more information).

\lstinputlisting[
    language=MPD,
    caption={
        [\acs{MPD} snippet with location addresses]
        Snippet of \acs{MPD} using the file path addresses. For the \acs{MPD} in its entirety, see appendix \ref{app:mpd}},
    firstnumber=25, 
    firstline=25, 
