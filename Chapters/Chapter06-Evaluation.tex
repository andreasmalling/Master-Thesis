...
The video used in all experiments is Big Buck Bunny\footnote{\url{https://peach.blender.org/download/}}, shortened to show only the first 90 \ac{s} of playback and thereby making the experiments faster to run. The video is by default divided in a segment size of approximately 3000 \ac{ms} resulting in 30 segments of video data plus an inittial container file. All segments amounts to the size of 55.99 \ac{MB} (55992320 bytes), yielding an average bitrate of 4919 \ac{kbps}, with individual segment sizes ranging from below 450 \ac{kB} to as high as 4.1 \ac{MB}.
...
\section{Mobile impact experiments}
...
Decreasing the bandwidth would result in not having enough for just watching the video alone from a single source, without any other network communication.
...
\section{Socialness impact experiments}
...
\section{Increased segment duration experiments}
...
% \end{itemize}
\ac{IPFS} has several shortcomings when it comes to performance, the first of which is its large \acs{CPU} usage, which significantly limits how many peers that can be simulated, this issue is further worsened by the large \acs{CPU} usage of the \ac{DASH} video player. This \ac{CPU} usage could potentially be lowered using other operating systems as Linux does not have accelerated video decoding available.\footnote{https://bugs.chromium.org/p/chromium/issues/detail?id=137247}
Despite the small network another scaling issue of \ac{IPFS} was apparent. As the number of peers hosting a file grew, the amount of duplicate data did as well. This overhead meant that lowering the bandwidth to 10 \ac{Mbps} resulted in very poor \ac{QOE}, despite the fact that 5 \ac{Mbps} should have been enough in theory. Combine this with the fact that adding latency and/or using the global \ac{IPFS} network, further increases the amount of duplicate data, and its clear that \ac{IPFS} is ill suited for mobile devices as they have worse network capabilities.
There are however ways to lessen the amount of duplicate data a user receives, the first is to have users only share their file segments while they are still streaming the video, with this strategy less peers serve the file segments at any given time. However this strategy is also dangerous as the segments corresponding to the later parts of the video, are not stored for very long on a peer before it leaves the network, which means these segments could potentially disappear from the network if there were no one to seed them. Another way to reduce the data duplication was to have the users have larger time between when they start the video, the cause of this is unknown but a possible explanation could be that the bitswap of \ac{IPFS} buts much effort in distributing files that a peer recently received. This phenomenom makes \ac{IPFS} fairly similar to a centralized system as it does not handle sudden large influx in users, similar to a \ac{DDoS} attack.\footnote{DDOS maybe link to wikipedia??????}
The final way to reduce duplicate was to increase the segment duration of the dashed video files, this comes at the cost of increasing the initial wait time before the video can start playing. This behaviour shows that \ac{IPFS} is more suited for retrieving few large files rather than many small ones.
In terms of user behaviours the negative experiences caused by these were mostly isolated within the user that caused these. Leeching resulted in frequent stalling on the leecher, while the other users could continue normally until the percentage of leechers was so large that the seeder was overloaded. In terms of viewing patterns skipping in the video resulting in stalls during the skip, but had no negative impact on the other users in the network.
